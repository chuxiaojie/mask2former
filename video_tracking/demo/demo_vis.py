...
def inference_vis(model: nn.Module, imgs: np.ndarray) -> SampleList:
    """Inference image(s) with the mot model.

    Args:
        model (nn.Module): The loaded mot model.
        imgs (List of np.ndarray): Loaded image.
    ...
    for img in imgs:
        data = dict(
        img=img.astype(np.float32), ori_shape=img.shape[:2])
    ...
    # forward the model
    with torch.no_grad():
        data = mmengine.dataset.default_collate(list_data)
...
def main(args):
    ...
    print("\nRuning Model")
    result = inference_vis(model, list_imgs)
